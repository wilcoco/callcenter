...
배정 원칙:
- 전화 건 사람(구성원)이 특정 팀이나 임원을 직접 지정했으면 반드시 그 팀/임원에게 배정하세요.
- 지정이 없으면 내용의 맥락으로 판단해 배정하세요.
- 일상적인 업무·문의는 반드시 실무 팀에 배정하세요.
- [임원]/[대표] (key가 exec_ 또는 ceo_로 시작)는 경영 판단이 필요한 중대 사안,
  법적 분쟁, 대형 사고·리콜, 임원/대표 면담 요청일 때만 배정하세요.
- 어느 쪽인지 모호하면 실무 팀 또는 general을 선택하세요.
...
{{"team_key": "<위 key 중 하나>",
  ...
  "caller_name": "<전화 건 사람의 소속/이름. 언급 없으면 빈 문자열>",
  "callback": "<통화 중 '다른 번호로 회신해달라'며 불러준 전화번호(숫자만). 지금 거신 번호로 하기로 했거나 언급 없으면 빈 문자열>"}}"""
    ...
    if settings.llm_enabled and transcript.strip():
        try:
            ...
            return {
                ...
                "caller_name": str(data.get("caller_name", "")).strip()[:128],
                "callback": str(data.get("callback", "")).strip()[:64],
            }
...
def _fallback_analysis(transcript: str) -> dict:
    ...
    phone_match = _PHONE_RE.search(basis)
    return {
        ...
        "caller_name": "",
        "callback": re.sub(r"[ .-]", "", phone_match.group()) if phone_match else "",
    }
