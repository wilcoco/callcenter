...
## 회사 지식 문서로 응대 내용 지정하기 (`knowledge.md`)

AI가 대답할 내용을 **문서로 미리 제공**할 수 있습니다. 저장소 루트의 `knowledge.md` 에
회사 정보·FAQ·정책(영업시간, 환불 규정, 요금제 등)을 적어두면, AI 상담원이 통화 중
그 문서를 근거로 답변합니다.

- 파일을 수정하면 **서버 재시작 없이 다음 통화부터 즉시 반영**됩니다.
- 문서에 없는 내용은 추측하지 않고 "담당 팀 확인 후 연락드리겠다"고 안내하도록 지시되어 있습니다.
- 경로는 `.env` 의 `KNOWLEDGE_FILE` 로 변경 가능합니다.
- 문서가 아주 큰 경우(수백 페이지 이상)는 프롬프트 주입 대신 RAG(검색 증강) 방식 도입을 권장합니다
  (현재는 약 5만 자까지 사용하고 초과분은 잘립니다).
